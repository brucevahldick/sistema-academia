...
function InstrutoresForm({aluno, readonly}: Props) {
    ...
    const handleSubmit = async (event: FormEvent) => {
        ...
        if (formValues.inputId) {
            if (window.confirm("Deseja editar o aluno " + formValues.inputNome + "?")) {
                makeApiPostCall(PORTA_INSTRUTOR, handleSuccess, handleError, formValues)
            }
        } else {
            makeApiPutCall(PORTA_INSTRUTOR, handleSuccess, handleError, formValues)
        }
    };
...
export default InstrutoresForm